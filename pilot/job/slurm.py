...
class Job(object):
    ...
    def __init__(self, job_description, resource_url, pilot_compute_description=None):
        ...
        self.bootstrap_script = self.job_description["executable"]
        if self.job_description.has_key("arguments"):
            self.bootstrap_script = (("%s %s") % (self.job_description["executable"],
                                                  self.job_description["arguments"]))
        self.job_id = ""
        self.resource_url = resource_url
        #self.resource_url.scheme = "ssh"
        logger.debug("Pilot-Streaming SLURM: Parsing job description")
        if pilot_compute_description == None: pilot_compute_description = {}
        if job_description.has_key('queue'): pilot_compute_description['queue'] = job_description['queue']
        if job_description.has_key('project'): pilot_compute_description['project'] = job_description['project']
        pilot_compute_description['working_directory'] = os.getcwd()
        if job_description.has_key('working_directory'): pilot_compute_description['working_directory'] = job_description['working_directory']
        if job_description.has_key('walltime'): pilot_compute_description['walltime'] = job_description['walltime']
        pilot_compute_description['number_of_processes'] = 1
        if job_description.has_key('total_cpu_count'): pilot_compute_description['number_of_processes'] = job_description['total_cpu_count']
...
""") % (str(pilot_compute_description["number_of_processes"]), str(walltime_slurm),
        str(pilot_compute_description["project"]), pilot_compute_description["working_directory"],
        pilot_compute_description["working_directory"], pilot_compute_description["queue"],
        pilot_compute_description["working_directory"], self.bootstrap_script)
        ### escaping characters
...
#
        ...
        logger.debug("Submit pilot job to: " + str(self.resource_url))
        start_command = ("python -c %s"%self.bootstrap_script)
